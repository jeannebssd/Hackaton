...
# Dictionnaire, ensemble de clé et de valeur (la plupart du temps, la clé est une chaîne de caractères)
DIRECTIONS = {
    "DOWN": (0, +1),
    "UP": (0, -1),
    "RIGHT": (+1, 0),
...
# Pour savoir si le serpent est encore dans l'écran
def in_scope(tile):
    x, y = tile
    return 0 <= x < X and 0 <= y < Y


# Technique du f-string, parenthèses pas obligatoires
def quit(snake, reason):
    print(f"Game over ({reason}) with a score of {len(snake)}")
    pg.quit()
    exit()

cases_vides = [case for case in product(range(X), range(Y)) if case not in snake]
print(cases_vides)
